...
| nessus\_bucket | The name of the S3 bucket where the production Nessus package lives. | `string` | n/a | yes |
| nessus\_package\_pattern | The pattern that matches the name of Nessus package objects in the S3 bucket specified by the nessus\_bucket variable. | `string` | `"Nessus-*.*"` | no |
| tags | Tags to apply to all AWS resources created | `map(string)` | ```{ "Application": "ansible-role-nessus testing", "Team": "VM Fusion - Development" }``` | no |
...
| bucket\_policy | The IAM policy that allows the CI user to read certain objects in the third-party S3 bucket. |
